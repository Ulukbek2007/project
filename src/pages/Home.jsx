...
{ data.length>0?     <div>
      <div className='home-banner-one'>
        ...
        <div className='home-banner-sale'>
          <div className='home-banner-relative'>
            ...
            <h2>LENNON R2D2 <br /> WITH NVIDIA 5090 TI</h2>
            <button  className='button'><a style={{textDecoration:'none',color:'white'}} href="#home-cart">Shop Now</a></button>
          </div>

          <img className='home-banner-laptop' src={laptop} alt="" />
        </div>
      </div>
      <div className='home-banner-two'>
        <h2>Trending</h2>
        <div id='home-cart' className='home-container-cart'>
          {!more ? fiveData.length > 0 && fiveData.map((el) => (
            <div key={el.id} className='home-cart'>
              <Link to={`/detail/:${el.id}/:${el.category.name}`} className='home-cart-to-detail'>
                <img src={el.images[0]} alt="" />
                <div className='home-cart-product-text'>
                  <div className='home-cart-product-name'>
                    <h2>{el.category.name}</h2>
                    <p>{el.title.length > 25 ? `${el.title.slice(0, 25)}...` : el.title}</p>
                  </div>
                  <div className='home-cart-product-price'><h2>{el.price}$</h2><h4>{el.price + 20}$</h4></div>
                </div>
              </Link>
            </div>
          ))  : data.length > 0 && data.map((el) => (
            <div key={el.id}>{el.images[0].slice(0, 1) !== '[' && <div className='home-cart'>
              <Link to={`/detail/:${el.id}/:${el.category.name}`} className='home-cart-to-detail'>
                <img src={el.images[0]} alt="" />
                <div className='home-cart-product-text'>
                  <div className='home-cart-product-name'>
        ...
        {fiveData.length > 0 && <button className='button' onClick={() => setMore(!more)}>{!more ? 'See more' : 'See less'}</button>}
      </div>
      <div className='home-banner-three'>
        <h2>Worth seeing</h2>
        <div className='home-worth-seeing'>
          {data.length > 0 && data.slice(1, 6).map(el => (
            <div key={el.id}>{el.images[0].slice(0, 1) !== '[' && <div className='home-seeing'>
              <Link to={`/detail/:${el.id}/:${el.category.name}`} className='home-cart-to-detail'>
                <img src={el.images[0]} alt="" />
      ...
      <div className='home-banner-four'>
        <div className='home-banner-four-button'>
          <img className='home-cross' src={cross} alt="" />
          <div>
            <p className='home-banner-four-new-year-text'>NEW YEAR</p>
            <p className='home-banner-four-sale-text'>SALE</p>
            <button className='button'>See more</button>
          </div>
          <img className='home-playstation' src={playstation} alt="" />
        </div>
        <div className='home-four-banner-sale'>
          <p className='home-four-banner-sale-one-text'>save up to  off</p><p className='home-four-banner-sale-two-text'>50%</p>
        </div>
      </div>
      <div className='home-banner-five'>
        <h2>Less than 100$</h2>
        <div className='home-container-cart'>
          {!less ? data.length > 0 && data.filter(el => el.price < 100).slice(0, 5).map((el) => (
            <div key={el.id} className='home-cart'>
              <Link to={`/detail/:${el.id}/:${el.category.name}`} className='home-cart-to-detail'>
                <img src={el.images[0]} alt="" />
                <div className='home-cart-product-text'>
          ...
          )) : data.length > 0 && data.filter(el => el.price < 100).map((el) => (
            <div key={el.id}>{el.images[0].slice(0, 1) !== '[' && <div className='home-cart'>
              <Link to={`/detail/:${el.id}/:${el.category.name}`} className='home-cart-to-detail'>
                <img src={el.images[0]} alt="" />
                <div className='home-cart-product-text'>
